#include "Radix.hpp"
#include "Hucre.hpp"
#include<iomanip>
Radix::Radix(Hucre* sayilar,int adet)
{
    this->sayiAdedi = adet;

    this->sayilar = new int[sayiAdedi];

    for(int i=0;i<sayiAdedi;i++)
    {
        this->sayilar[i] = sayilar->veri;
        sayilar = sayilar->sonraki;
    }

    Queuelar = new Queue*[10];
    for(int i=0;i<10;i++)
        Queuelar[i] = new Queue();
}
Radix::~Radix()
{
    delete[] sayilar;
    for(int i=0;i<10;i++)
        delete Queuelar[i];
    
    delete [] Queuelar;
}

int *Radix::sirala()
{

    for(int i=0;i<sayiAdedi;i++)
    {
        int sonBasamak = sayilar[i]%10;

        Queuelar[sonBasamak]->ekle(sayilar[i]);

    }
    int maxBasamakSayisi = maxBasamakSayisiGetir();
    
    
    int basamakCarpani = 10;

    for(int i = 1;i<maxBasamakSayisi;i++ )
    {
        int uzunluklar[10];

        QueueUzunluklariGetir(uzunluklar);

        for(int j=0;j<10;j++)
        {
            int es = uzunluklar[j];

            while(es)
            {
                int siradakiSayi = Queuelar[j]->getir();

                Queuelar[j]->cikar();
                int basamak = (siradakiSayi/basamakCarpani)%10;
                Queuelar[basamak]->ekle(siradakiSayi);
                es--;
            }
        }
        basamakCarpani*=10;
    }
    int diziIndex = 0;
    for(int i=0;i<10;i++)
    {
        while(!Queuelar[i]->bosmu())
        {
            sayilar[diziIndex] = Queuelar[i]->getir();
            Queuelar[i]->cikar();
            diziIndex++;
        }
    }
    return sayilar;
}

int Radix::maxBasamakSayisiGetir()
{
    int maxBasamakSayisi = basamakSayisiGetir(sayilar[0]);

    for(int i=1;i<sayiAdedi;i++)
    {
        int siradakiBasamakSayisi = basamakSayisiGetir(sayilar[i]);
        if(maxBasamakSayisi<siradakiBasamakSayisi)
            maxBasamakSayisi = siradakiBasamakSayisi;
    }
    return maxBasamakSayisi;
}
int Radix::basamakSayisiGetir(int sayi)
{
    int basamakSayisi = 0;
    while(sayi)
    {
        sayi = sayi/10;
        basamakSayisi++;
    }
    return basamakSayisi;
}
void Radix::QueuelariYazdir()
{
    for(int i=0;i<10;i++)
    {
        cout<<setw(5)<<std::left<<i<<":"<<*Queuelar[i];
    }
    cout<<"------------------------"<<endl;
}

void Radix::QueueUzunluklariGetir(int* uzunluklar)
{
    for(int i=0;i<10;i++)
    {
        uzunluklar[i] = Queuelar[i]->elemanSayisiGetir();
    } 
}